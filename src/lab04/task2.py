...
def print_summary(
    tokens: List[str], top_words: List[Tuple[str, int]], top_n: int = 5
) -> None:
    """
    ...
    print(
        f"Топ-{min(top_n, len(top_words))}: {[word for word, count in top_words[:top_n]]}"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Анализ текста и генерация отчета о частотах слов"
    )
    parser.add_argument(
        "--in",
        dest="input_file",
    ...
    parser.add_argument(
        "--top", type=int, default=5, help="Количество слов для топа (по умолчанию: 5)"
    )
    ...
    try:
        ...
        print(
            "Попробуйте указать правильную кодировку через --encoding (например, --encoding cp1251)"
        )
        sys.exit(1)
