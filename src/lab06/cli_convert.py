...
def main():
    ...
    json2csv_parser.add_argument(
        "--in", dest="infile", required=True, help="Входной JSON файл"
    )
    json2csv_parser.add_argument(
        "--out", dest="outfile", required=True, help="Выходной CSV файл"
    )

    # Подкоманда csv2json
    csv2json_parser = subparsers.add_parser("csv2json", help="Конвертация CSV в JSON")
    csv2json_parser.add_argument(
        "--in", dest="infile", required=True, help="Входной CSV файл"
    )
    csv2json_parser.add_argument(
        "--out", dest="outfile", required=True, help="Выходной JSON файл"
    )
    ...
    csv2xlsx_parser.add_argument(
        "--out", dest="outfile", required=True, help="Выходной XLSX файл"
    )
